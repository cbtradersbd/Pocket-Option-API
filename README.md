# ⚡ Pocket Option 24/7 OTC REST API, WebSocket Streamer & AI Signal Engine

> **🚀 High-throughput Pocket Option Python Async REST API & WebSocket wrapper. 24/7 OTC live prices, closed M1 candle history & auto-trading bridge.**
...
- 🕒 **24/7 OTC Market Coverage**: Uninterrupted weekend and night feeds.
- 📊 **Rolling 8-Day History**: Non-repainting closed candle storage.
- 🎯 **TradingView Integration**: Direct Pine Script webhook execution.
...
| `/api/pocketoption/live-price` | `GET` | Current bid/ask and tick stream | JSON |
| `/api/pocketoption/candles` | `GET` | Historical closed OHLCV candle records | JSON Array |
| `/api/pocketoption/payouts` | `GET` | Live payout percentage monitor for all pairs | JSON Object |
| `/api/pocketoption/signals` | `POST` | Execute automated strategy webhook trigger | JSON Result |
| `/ws/pocketoption/stream` | `WSS` | Ultra-low latency binary WebSocket feed | Binary / JSON |
